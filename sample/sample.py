...
# MECA DAO initiator deploys all contract and publishes the addresses
...
# %%
# Other MECA DAO entities join the DAO with their on-chain identity
...
# %%
# meca user checks the blockchain to determine the tower and host
...
# %%
# meca user proposes a task request with the selected tower and host.
...
# the input is encrypted with the hosts public key and sent to the tower

# %%
# execution of the task
# 1. tower receives the task request from user and stores the input locally (not shown here)
# 2. tower checks the task to ensure it is valid (Validity can include tower address, host address etc.)
print(meca_tower.get_running_task(task_id=task_id))
# 3. tower sends the task to the host (not shown here)
# 4. host retrieves the input from tower and checks the task is valid (not shown here)
print(meca_host.get_running_task(task_id=task_id))
# 5. host decrypts the input with its private key and executes the task (not shown here)
# 6. host stores the output locally (not shown here)


# %%
# meca user pulls the results from the tower
# meca user acknowledges the task request completion
...
# %%
# host deletes its active task, empties its queue and leaves MECA DAO
...
# %%
# tower empties its queue and leave MECA DAO
